...
# Exercise 6
# The paper proposed that there are canonical (MATCH > 0.75) and non-canonical (MATCH < 0.75) 11-mer motif of NF-kB. We want to use both (canonical and non-canonical) sequences to align back to our ChIP-seq result to validate whether our data are reasonable, by counting how many sequences got aligned. They proposed that canonical sequence is RGGRNNHHYYB, and non-canonical sequences can be found in figure 4b, varying with different dimer formation.To align, first we have to separate positive and negative strands of our fasta file, this can be achieved by "grep -A". Then we use "grep -E" to match the canonical or non-canonical sequences to the fasta file of us. Below are some codes to achieve above goal:
